...
{
	internal class MultiColumnTreeView : TreeViewWithTreeModel<ObstacleTreeElement>
	{
		...
		public MultiColumnTreeView (TreeViewState state, MultiColumnHeader multicolumnHeader, TreeModel<ObstacleTreeElement> model) : base (state, multicolumnHeader, model)
		{
		...
		void SortByMultipleColumns ()
		{
			...
			var myTypes = rootItem.children.Cast<TreeViewItem<ObstacleTreeElement> >();
		...
		IOrderedEnumerable<TreeViewItem<ObstacleTreeElement>> InitialOrder(IEnumerable<TreeViewItem<ObstacleTreeElement>> myTypes, int[] history)
		{
        ...
        public override void OnGUI(Rect rect)
        {
			...
			if (Event.current.type == EventType.DragExited)
			{
                ...
                {
					List<ObstacleTreeElement> treeElements = ObstacleTreeElementGenerator.GetDraggingElements(this.treeModel);

					this.treeModel.AddElements(treeElements, this.treeModel.root, 0);
		...
		private void RightClickMenu(Rect rect)
        {
			...
			if (e.type == EventType.ContextClick)
			{
				...
				if (rect.Contains(mosPosition))
				{
					...
					menu.AddItem(new GUIContent("Remove selected"), false, RemoveSelected, "Remove selected");
					menu.AddItem(new GUIContent("Remove all"), false, RemoveAll, "Remove all");
					menu.ShowAsContext();
					e.Use();
				}
			}
		...
		private void RemoveSelected(object obj)
		{
			...
			this.treeModel.RemoveElements(selections);
		}
		private void RemoveAll(object obj)
        {
			this.treeModel.ClearElements();
		}

		protected override void RowGUI (RowGUIArgs args)
		{
			var item = (TreeViewItem<ObstacleTreeElement>) args.item;
		...
		void CellGUI (Rect cellRect, TreeViewItem<ObstacleTreeElement> item, MyColumns column, ref RowGUIArgs args)
		{
			...
			switch (column)
			{
				...
				case MyColumns.GameObject:
				case MyColumns.FilePath:
					{
						if (showControls)
						{
							...
							if (column == MyColumns.GameObject)
								item.data.gameObject = (GameObject)EditorGUI.ObjectField(cellRect, GUIContent.none, item.data.gameObject, typeof(GameObject), false);
							if (column == MyColumns.FilePath)
								/*item.data.text = */GUI.Label(cellRect, item.data.filePath);
						}
						else
						{
		...
		public static MultiColumnHeaderState CreateDefaultMultiColumnHeaderState(float treeViewWidth)
		{
			var columns = new[] 
			{
				...
				new MultiColumnHeaderState.Column 
				{
					headerContent = new GUIContent("GameObject", "Maecenas congue non tortor eget vulputate."),
					headerTextAlignment = TextAlignment.Right,
					sortedAscending = true,
					sortingArrowAlignment = TextAlignment.Left,
					width = 95,
				...
				new MultiColumnHeaderState.Column 
				{
					headerContent = new GUIContent("FilePath", "Nam at tellus ultricies ligula vehicula ornare sit amet quis metus."),
					headerTextAlignment = TextAlignment.Right,
					sortedAscending = true,
					sortingArrowAlignment = TextAlignment.Left,
					width = 70,
