...
namespace UnityEditor.TreeViewExamples
{
	...
	[CustomEditor (typeof(ObstacleTreeAsset))]
	public class ObstacleTreeAssetEditor : Editor
	{
		MyTreeView m_TreeView;
		SearchField m_SearchField;
		const string kSessionStateKeyPrefix = "TVS";

		ObstacleTreeAsset asset
		{
			get { return (ObstacleTreeAsset) target; }
		}
		...
		void OnEnable ()
		{
			...
			var treeModel = new TreeModel<ObstacleTreeElement> (asset, asset.treeElements);
		...
		public override void OnInspectorGUI ()
		{
			if (Event.current.type == EventType.DragExited)
			{
				...
				List<ObstacleTreeElement> treeElements = ObstacleTreeElementGenerator.GetDraggingElements(this.m_TreeView.treeModel);

				m_TreeView.treeModel.AddElements(treeElements, this.m_TreeView.treeModel.root, 0);

				//m_TreeView.treeModel.MyTreeAsset.treeElements = treeElements;
		...
		void ToolBar ()
		{
			using (new EditorGUILayout.HorizontalScope ())
			{
				...
				if (GUILayout.Button ("Add Item", style))
				{
					...
					var element = new ObstacleTreeElement ("Item " + id, depth, id, null, "");
		...
		class MyTreeView : TreeViewWithTreeModel<ObstacleTreeElement>
		{
			public MyTreeView(TreeViewState state, TreeModel<ObstacleTreeElement> model)
				: base(state, model)
			{
				showBorder = true;
