...
namespace UnityEditor.TreeViewExamples
{
	...
	public class TreeModel<T> where T : TreeElement
	{
		...
		private ObstacleTreeAsset m_myTreeAsset;

		public ObstacleTreeAsset MyTreeAsset { get { return m_myTreeAsset; } }
		...
		public TreeModel (ObstacleTreeAsset myTreeAsset, IList<T> data)
		{
			this.m_myTreeAsset = myTreeAsset;
			SetData (data);
		}
		...
		public void ClearElements()
        {
			...
			IList<T> elements = m_Data.Where(element => element.id != 0 ).ToArray();
		...
		public void AddElements (IList<T> elements, TreeElement parent, int insertPosition)
		{
			...
			if (elements.Count == 0)
				UnityEngine.Debug.LogWarning("elements Count is 0: nothing to add");
			if (parent == null)
			...
			//parent.children.Where(x=>((MyTreeElement)x).gameObject )

			var rightList = elements.Cast<ObstacleTreeElement>().ToList();
			var leftList = parent.children.Select(x => (ObstacleTreeElement)x);

			List<ObstacleTreeElement> targetList = new List<ObstacleTreeElement>();

			var rightOuterJoin = rightList.GroupJoin(leftList,
				right => right.gameObject,
				left => left.gameObject,
				(right, left) => new { TableA = right, TableB = left }
				);

			//var bb = rightOuterJoin.ToList();

            foreach (var item in rightOuterJoin)
            {
                if (item.TableB.Count() == 0)
                {
                    targetList.Add(item.TableA);
                }
            }

            //var aa = targetList.Count;


            parent.children.InsertRange(insertPosition, targetList);
			foreach (var element in elements)
			{
		...
		void Changed ()
		{
			...
			MyTreeAsset.treeElements = m_Data.Cast<ObstacleTreeElement>().ToList();
			EditorUtility.SetDirty(MyTreeAsset);
	...
	class TreeModelTests
	{
		...
		public static void TestTreeModelCanAddElements()
		{
			...
			var model = new TreeModel<TreeElement>(null, listOfElements);
